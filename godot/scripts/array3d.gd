...
var _surface_data := PoolIntArray()
...
func set_height(x: int, z: int, height: float) -> void:
	_height_data[((x + radius_x) << size_z_pow) + z + radius_z] = height
#	return
...
func get_height(x: int, z: int) -> float:
	...
	return _height_data[((x + radius_x) << size_z_pow) + z + radius_z]
#	var saved_height := _height_data[((x + radius_x) << size_z_pow) + z + radius_z]
...
func set_surface_value(x: int, z: int, value: int) -> void:
	_surface_data[((x + radius_x) << size_z_pow) + z + radius_z] = value


func set_surface_value_by_idx(idx: int, value: int) -> void:
	_surface_data[idx] = value


func get_surface_value(x: int, z: int) -> int:
	if x < -radius_x or x >= radius_x \
	or z < -radius_z or z >= radius_z:
		return 0
	return _surface_data[((x + radius_x) << size_z_pow) + z + radius_z]
...
func set_radius(x: int, y: int, z: int) -> void:
	...
	_surface_data.resize(size_x * size_z)
